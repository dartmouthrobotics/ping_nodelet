...
#ifndef PING_NODELET_PING_MESSAGE_H
#define PING_NODELET_PING_MESSAGE_H
...
public:
    ...
    ping_message(const ping_message &msg)
        ...
        , msgData { reinterpret_cast<uint8_t*>(malloc(sizeof(uint8_t) * _bufferLength)) }
    {
        memcpy(msgData, msg.msgData, _bufferLength);
    }

    explicit ping_message(const uint16_t bufferLength)
...
public:
    ...
    uint16_t msgDataLength() const
    {
        ...
        return static_cast<uint16_t>(headerLength + payload_length() + checksumLength);
    }
    ...
    {
        return static_cast<uint16_t>(msgData[msgDataLength() - checksumLength] +
            (msgData[msgDataLength() - checksumLength + 1] << 8));
    }
    void set_checksum(uint16_t checksum)
    {
        msgData[msgDataLength() - checksumLength] = (uint8_t)checksum;
        msgData[msgDataLength() - checksumLength + 1] = static_cast<uint8_t>(checksum >> 8);
    }
...
#endif  // PING_NODELET_PING_MESSAGE_H
